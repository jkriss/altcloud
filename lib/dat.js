...
const serve = function(root) {
  Dat(root, function (err, dat) {
    ...
    if (dat.writable) {
      ...
      //   console.log('IMPORT del:', src.name)
      // })
...
const mirror = function(datKey, root) {
  ...
  Dat(root, {
    ...
    stats.on('update', function() {
      console.log(`altcloud dat: ${stats.peers.total} peers (${stats.peers.complete} complete), download speed ${stats.network.downloadSpeed}`)
    })
  })
}
