...
const debug = require('debug')('altcloud:access:enforcement')
...
module.exports = function(opts) {
  return function(req, res, next) {
    ...
    debug(`user ${user}, rules`, rules)
    // "all" allows everybody, authed or not
    if (rules && rules.indexOf('all') !== -1) return next()
    // "nobody" blocks all users, authed or not
    if (rules && rules.indexOf('nobody') !== -1) return next(createError(404))
    // some files should *always* be hidden
    if (req.path.indexOf('/.keys/private.key') === 0)
      return next(createError(404))
    // if no rules, assume special files are hidden
    if (!rules && Path.basename(req.path)[0] === '.')
      return next(createError(404))
    // let middleware mark a response as hidden by default, too
    if (!rules && req.altcloud && req.altcloud.hiddenByDefault)
      return next(createError(404))
