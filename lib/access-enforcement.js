...
module.exports = function (opts) {
  ...
  return function (req, res, next) {
    ...
    if (!rules && ['.access', '.passwords', '.tokens'].indexOf(Path.basename(req.path)) !== -1) return next(createError(404))
    // let middleware mark a response as hidden by default, too
    if (!rules && req.altcloud.hiddenByDefault) return next(createError(404))
