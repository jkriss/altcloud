...
const debug = require('debug')('altcloud:layouts')
...
module.exports = function(opts) {
  return function(req, res, next) {
    ...
    if (
      req.altcloud &&
      req.altcloud.attributes &&
      req.altcloud.attributes.layout
    ) {
      const layoutPath = Path.join(
        opts.root,
        Path.dirname(req.altcloud.actualPath || req.path),
        req.altcloud.attributes.layout
      )
      ...
      fs.readFile(layoutPath, 'utf8', function(err, templateString) {
        ...
        debug(
          `rendering ${req.path} with layout ${req.altcloud.attributes.layout}`
        )
        const data = Object.assign({}, req.altcloud.attributes, {
          content: req.altcloud.fileContents
        })
        if (req.altcloud.params) data.params = req.altcloud.params
        if (req.user)
          data.currentUser = req.user(`rendering with params`, data.params)
        engine
